...
class PyTorchFederatedHistoCNN(PyTorchTaskRunner):
    ...
    def train_epoch(self, batch_generator: Iterator[Tuple[torch.Tensor, dict]]) -> Metric:
        ...
        losses_dict = self.run_train_or_val(subset='train', loader=batch_generator, epoch=self.epoch)
    ...
    def validate(self, col_name, round_num, input_tensor_dict,
        ...
        losses_dict = self.run_train_or_val(subset='val', loader=loader, epoch=epoch)
    ...
    def run_train_or_val(self, subset='train', loader=None, epoch=0):
        all_risk_scores = []
        all_censorships = []
        all_event_times = []
        ...
        if self.has_surv:
            ...
            c_index = concordance_index_censored((1 - all_censorships).astype(bool), all_event_times, all_risk_scores,
                                                 tied_tol=1e-08)[0]
            print('{}_c_index: {:.4f}'.format(subset, c_index))
        else:
            c_index = 0
        ...
        for k in losses_dict.keys():
            ...
            if self.writer:
                self.writer.add_scalar('{}/loss_{}'.format(subset, k), losses_dict[k], epoch)
                self.writer.add_scalar('{}/c_index'.format(subset), c_index if self.has_surv else 0, epoch)
                self.writer.add_scalar('{}/total_loss'.format(subset), sum([v for v in losses_dict.values()]), epoch)

        print('Epoch {}, {}'.format(
            epoch, ', '.join(['{}: {:.4f}'.format(k, v) for k, v in losses_dict.items()])))

        print('{} cls accs:'.format(subset))
        for name, labels in self.classification_dict.items():
            ...
            for average in ['micro', 'macro', 'weighted']:
                ...
                if self.writer:
                    self.writer.add_scalar('{}/{}_f1_{}'.format(subset, name, average), score, epoch)
            ...
            for average in ['macro', 'weighted']:
                ...
                if self.writer:
                    self.writer.add_scalar('{}/{}_auc_{}'.format(subset, name, average), auc, epoch)

            print('generated ROC curves ...')
            loggers_dict[name].get_roc_curve(
                os.path.join(self.save_dir, 'epoch_{:03}_{}_{}_ROC.jpg'.format(epoch, name, subset)))

            print('save data')
            loggers_dict[name].save_data(
                os.path.join(self.save_dir, 'epoch_{:03}_{}_{}_data.txt'.format(epoch, name, subset)))

            print('generate classification report')
            loggers_dict[name].get_classification_report(
                os.path.join(self.save_dir, 'epoch_{:03}_{}_{}_report.txt'.format(epoch, name, subset)))
            ...
            for j in range(len(labels)):
                ...
                if self.writer:
                    self.writer.add_scalar('{}/{}_{}_{}_acc'.format(subset, name, j, self.classification_dict[name][j]),
                                           acc, epoch)

        print('{} reg mses:'.format(subset))
        for name in self.regression_list:
            ...
            if self.writer:
                self.writer.add_scalar('{}/{}_mse'.format(subset, name), mse, epoch)
